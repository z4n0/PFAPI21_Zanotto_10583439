...
void consolidate(FibHeap* H){
    ...
    FibNode* lastNode = currentRootNode->left; //The final node we will consolidate
...
//            if (y == H->min) //se prima di scambiarsi x(rootNodeCurrent) era hmin dopo lo scambio y sara hmin ma dopo la fusione hmin diventera lelem con valore minore che ora è puntato da x (current root node)se quello che sta per diventare figlio era hmin allora lo diventa il suo padre->ergo x
//                H->min = rootNodeCurrent;
            fib_Heap_Link(y,x);
...
FibNode* fib_Extract_Min(FibHeap *H) {
    FibNode *nodeToExtract = H->min; //is the node to extract
